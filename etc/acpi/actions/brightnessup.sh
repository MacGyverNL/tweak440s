...
# Now find out what the brightness actually was. Since we're increasing, it will be easy to round:
# Assuming we never read when it's already increased by 5, this will give the correct value.
BRIGHTNESS=$(($BRIGHTNESS / 5 * 5))
...
[[ $BRIGHTNESS -lt 70 && $BRIGHTNESS -ge 20 || $BRIGHTNESS -le 5 ]] && BRIGHTNESSSTEP=5 || BRIGHTNESSSTEP=10
